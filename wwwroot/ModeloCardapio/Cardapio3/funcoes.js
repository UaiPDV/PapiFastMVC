...
window.onload = function () {
    ...
    FoneCliente = prompt("Digite o número de telefone!");
    FoneCliente = '55' + FoneCliente;
};
...
function enviar(ListaProd, Fonenumber) {
    var target;
    var mesa = document.querySelector("#NumMesa");
    ...
    if (isMobile) {
        ...
        let msg = `${data} %0A%0A`;
        for (var i = 0; i < ListaProd.length; i++) {
            msg += `Cod: ${ListaProd[i].codigo}%0A${ListaProd[i].quantidade} - ${ListaProd[i].NomeProd} ...... ${ListaProd[i].Valor}%0A%0A`;
        }
        var conta = document.querySelector(".conta").innerHTML;
        msg += `Total do pedido: ${conta}`;
...
async function sendRequest(message, phoneNumber, retryCount = 1) {
    return new Promise((resolve, reject) => {
        function makeRequest(retries) {
            const path = "https://1572-2804-4d98-25e-5400-555-213f-c91e-57af.ngrok-free.app/message";
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
                    if (xhr.status >= 200 && xhr.status < 300) {
                        ...
                        console.log("\x1b[31m%s\x1b[0m", "Erro in sendRequest", xhr.status);
                        if (retries > 0) {
            ...
            xhr.send(JSON.stringify({ message: message, phoneNumber: phoneNumber }));
        }
